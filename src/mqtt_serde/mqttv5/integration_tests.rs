...
mod tests {
    ...
    fn test_subscription_workflow_integration() {
        ...
        match MqttPacket::from_bytes(&subscribe_bytes).unwrap() {
            ParseOk::Packet(MqttPacket::Subscribe(parsed_subscribe), _) => {
                ...
                match MqttPacket::from_bytes(&suback_bytes).unwrap() {
    ...
    fn test_unsubscription_workflow_integration() {
        ...
        match MqttPacket::from_bytes(&unsubscribe_bytes).unwrap() {
            ParseOk::Packet(MqttPacket::Unsubscribe(parsed_unsubscribe), _) => {
                ...
                match MqttPacket::from_bytes(&unsuback_bytes).unwrap() {
    ...
    fn test_all_new_packet_types_through_control_system() {
        ...
        let subscribe_bytes = subscribe.to_bytes().unwrap();
        assert!(matches!(
            MqttPacket::from_bytes(&subscribe_bytes).unwrap(),
            ParseOk::Packet(MqttPacket::Subscribe(_), _)
        ));
        ...
        let suback_bytes = suback.to_bytes().unwrap();
        assert!(matches!(
            MqttPacket::from_bytes(&suback_bytes).unwrap(),
            ParseOk::Packet(MqttPacket::SubAck(_), _)
        ));
        ...
        let unsubscribe_bytes = unsubscribe.to_bytes().unwrap();
        assert!(matches!(
            MqttPacket::from_bytes(&unsubscribe_bytes).unwrap(),
            ParseOk::Packet(MqttPacket::Unsubscribe(_), _)
        ));
        ...
        let unsuback_bytes = unsuback.to_bytes().unwrap();
        assert!(matches!(
            MqttPacket::from_bytes(&unsuback_bytes).unwrap(),
            ParseOk::Packet(MqttPacket::UnsubAck(_), _)
        ));
        ...
        assert!(matches!(
            MqttPacket::from_bytes(&pingreq_bytes).unwrap(),
        ...
        assert!(matches!(
            MqttPacket::from_bytes(&pingresp_bytes).unwrap(),
        ...
        assert!(matches!(
            MqttPacket::from_bytes(&disconnect_bytes).unwrap(),
            ParseOk::Packet(MqttPacket::Disconnect(_), _)
        ));
        ...
        assert!(matches!(
            MqttPacket::from_bytes(&auth_bytes).unwrap(),
    ...
    fn test_ping_workflow_integration() {
        ...
        match MqttPacket::from_bytes(&pingreq_bytes).unwrap() {
            ParseOk::Packet(MqttPacket::PingReq(_parsed_pingreq), _) => {
                // Step 3: Server responds with PINGRESP
                let pingresp = MqttPingResp::new();
                let pingresp_bytes = pingresp.to_bytes().unwrap();

                // Step 4: Parse PINGRESP packet
                match MqttPacket::from_bytes(&pingresp_bytes).unwrap() {
                    ParseOk::Packet(MqttPacket::PingResp(_parsed_pingresp), _) => {
    ...
    fn test_disconnect_workflow_integration() {
        ...
        match MqttPacket::from_bytes(&disconnect_bytes).unwrap() {
            ParseOk::Packet(MqttPacket::Disconnect(parsed_disconnect), _) => {
        ...
        match MqttPacket::from_bytes(&disconnect_error_bytes).unwrap() {
            ParseOk::Packet(MqttPacket::Disconnect(parsed_disconnect), _) => {
    ...
    fn test_auth_workflow_integration() {
        ...
        match MqttPacket::from_bytes(&auth_bytes).unwrap() {
        ...
        match MqttPacket::from_bytes(&server_auth_bytes).unwrap() {
        ...
        match MqttPacket::from_bytes(&success_bytes).unwrap() {
